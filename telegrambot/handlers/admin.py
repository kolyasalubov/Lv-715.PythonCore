...
# @dp.message_handdler(commands = ['moderator'], is_chat_admin=True)
...
async def load_photo(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply("Теперь ведіть назву")
...
async def load_name(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply("Ведіть опис")
...
async def load_description(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply("Тепер ведіть ціну")
